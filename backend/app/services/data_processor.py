...
class DataProcessorService:
    ...
    def generate_enhanced_sample_data(self, db: Client, num_projects: int = 50, num_employees: int = 30):
    ...
    def _clear_existing_data(self, db: Client):
        """Clear existing data for fresh training"""
        try:
            db.table("daily_logs").delete().execute()
            db.table("sprints").delete().execute()
            db.table("projects").delete().execute()
            db.table("employees").delete().execute()
            logger.info("Cleared existing data")
        except Exception as e:
            logger.warning(f"Error clearing data: {e}")

    def _create_enhanced_employees(self, db: Client, num_employees: int) -> List[Dict]:
        ...
        for i in range(num_employees):
            ...
            employee = {
                ...
                "hire_date": (datetime.now() - timedelta(days=np.random.randint(180, 2000))).isoformat(),
                "created_at": datetime.now().isoformat(),
                "is_active": True
            }
            db.table("employees").insert(employee).execute()
            employees.append(employee)

        logger.info(f"Created {len(employees)} enhanced employees")
        return employees

    def _create_enhanced_projects(self, db: Client, num_projects: int) -> List[Dict]:
    ...
    def _create_enhanced_daily_logs(self, db: Client, projects: List[Dict], employees: List[Dict]):
        ...
        for days_ago in range(90):
            ...
            if days_ago % 10 == 0:
                logger.info(f"Processed logs for {days_ago} days ago")

        logger.info("✅ Enhanced daily logs created")

    def _get_employee_reliability(self, employee: Dict) -> float:
    ...
    def _calculate_realistic_hours(self, project: Dict, employee: Dict, date: datetime) -> float:
        ...
        elapsed_duration = (date - start_date).days
        total_duration = (end_date - start_date).days
    ...
    def _calculate_completion(self, project: Dict, hours: float) -> float:
    ...
    def _calculate_issues(self, project: Dict, hours: float) -> int:
    ...
    def _create_weekend_logs(self, db: Client, projects: List[Dict], employees: List[Dict],
    ...
    def _create_enhanced_sprints(self, db: Client, projects: List[Dict]):
        ...
        for project in projects:
            ...
            project_duration = (pd.to_datetime(project['end_date']) - pd.to_datetime(project['start_date'])).days
            num_sprints = max(2, int(project_duration / 21))  # 3-week sprints

            sprint_start = pd.to_datetime(project['start_date'])
    ...
    def generate_sample_data(self, db: Client, num_projects: int = 10, num_employees: int = 20):
        """Original method - now uses enhanced data generation"""
        return self.generate_enhanced_sample_data(db, num_projects, num_employees)

    def get_project_dashboard_data(self, db: Client, project_id: int) -> Dict[str, Any]:
        ...
        recent_logs_response = db.table("daily_logs").select("*").eq("project_id", project_id).gte("date", start_date).execute()
        recent_logs = recent_logs_response.data

        # Get sprint data
        sprints_response = db.table("sprints").select("*").eq("project_id", project_id).order("start_date").execute()
        ...
        start_date = pd.to_datetime(project['start_date'])
        end_date = pd.to_datetime(project['end_date'])
        days_elapsed = (datetime.now() - start_date).days if project['start_date'] else 0
        total_duration = (end_date - start_date).days if project['end_date'] and project['start_date'] else 0
